import { E as n, C as z, s as G, t as $, e as b, L as v, f as T, a as q, i as _, c as V, b as u, g as E } from "./index-3cbb0504.js";
const Y = 146, X = 147, C = 148, N = 1, F = 2, j = 3, I = 149, D = 150, h = 4, L = 5, K = 151, y = 152, k = 153, s = 154, W = 6, w = 7, A = 8, B = 9, c = 0, l = [
  9,
  10,
  11,
  12,
  ...
  8287,
  12288
], H = 58, M = 40, g = 95, J = 91, i = 45, OO = 46, m = 35, $O = 37, U = 123, eO = 125, r = 47, S = 42, d = 10, Z = 61, aO = 43;
function o(O) {
...
const QO = new n((O, e) => {
  if (e.dialectEnabled(c)) {
    let a;
    if (O.next < 0 && e.canShift(k))
      O.acceptToken(k);
    else if (((a = O.peek(-1)) == d || a < 0) && e.canShift(y)) {
      let Q = 0;
      for (; O.next != d && l.includes(O.next); )
        O.advance(), Q++;
      O.next == d || f(O) ? O.acceptToken(y, -Q) : Q && O.acceptToken(s);
    } else if (O.next == d)
      O.acceptToken(K, 1);
    else if (l.includes(O.next)) {
      for (O.advance(); O.next != d && l.includes(O.next); )
        O.advance();
      O.acceptToken(s);
    }
  } else {
    let a = 0;
    for (; l.includes(O.next); )
      O.advance(), a++;
    a && O.acceptToken(s);
  }
}, { contextual: !0 }), tO = new n((O, e) => {
  if (f(O)) {
      ...
      if (a > -1) {
        ...
        for (O.advance(); O.next >= 0; )
          if (O.next == d) {
            ...
            O.advance();
        O.acceptToken(Q ? w : W, t);
        return;
      }
    }
    if (O.next == r) {
      for (; O.next != d && O.next >= 0; )
        O.advance();
      O.acceptToken(W);
    } else {
      for (O.advance(); O.next >= 0; ) {
  ...
  (O.next == aO || O.next == Z) && e.dialectEnabled(c) && O.acceptToken(O.next == Z ? A : B, 1);
}), dO = new n((O, e) => {
  if (!e.dialectEnabled(c))
    return;
  ...
  if (O.peek(-1) == d) {
    ...
    for (; O.next != d && l.includes(O.next); )
      O.advance(), t++;
    t != a && O.next != d && !f(O) && (t < a ? O.acceptToken(X, -t) : O.acceptToken(Y));
  }
}), lO = new n((O, e) => {
  for (let a = !1, Q = 0, t = 0; ; t++) {
    let { next: R } = O;
    if (o(R) || R == i || R == g || a && p(R))
      !a && (R != i || t > 0) && (a = !0), Q === t && R == i && Q++, O.advance();
    else if (R == m && O.peek(1) == U) {
      O.acceptToken(L, 2);
      break;
    } else {
      a && O.acceptToken(R == M ? I : Q == 2 && e.canShift(h) ? h : D);
      break;
    }
  }
}), nO = new n((O) => {
  if (O.next == eO) {
    for (O.advance(); o(O.next) || O.next == i || O.next == g || p(O.next); )
      O.advance();
    O.next == m && O.peek(1) == U ? O.acceptToken(F, 2) : O.acceptToken(N);
  }
}), iO = new n((O) => {
  if (l.includes(O.peek(-1))) {
    let { next: e } = O;
    (o(e) || e == g || e == m || e == OO || e == J || e == H || e == i) && O.acceptToken(C);
  }
}), rO = new n((O) => {
  if (!l.includes(O.peek(-1))) {
    let { next: e } = O;
    if (e == $O && (O.advance(), O.acceptToken(j)), o(e)) {
      do
        O.advance();
      while (o(O.next));
      O.acceptToken(j);
...
const oO = new x(null, 0), SO = new z({
  start: oO,
  shift(O, e, a, Q) {
    return e == Y ? new x(O, a.pos - Q.pos) : e == X ? O.parent : O;
  },
  hash(O) {
    return O.hash;
  }
  ...
  tokenizers: [dO, iO, nO, rO, lO, QO, tO, RO, 0, 1, 2, 3],
  topRules: { StyleSheet: [0, 10], Styles: [1, 127] },
  dialects: { indented: 0 },
  specialized: [{ term: 150, get: (O) => sO[O] || -1 }, { term: 149, get: (O) => XO[O] || -1 }, { term: 72, get: (O) => PO[O] || -1 }],
  tokenPrec: 2798
}), P = /* @__PURE__ */ v.define({
  name: "sass",
...
function jO(O) {
  return new u(O != null && O.indented ? mO : P, P.data.of({ autocomplete: E }));
}
export {
  jO as sass,
  P as sassLanguage
